...
class SpeechRecognitionModel(nn.Module):
    ...
    # It takes spectrograms as input, and produces character probabilities per time step as output
...
# Simple WER calculation function
    ...
    for i in range(1, len(ref_words) + 1):
        for j in range(1, len(hyp_words) + 1):
            if ref_words[i - 1] == hyp_words[j - 1]:
                ...
                d[i][j] = min(d[i - 1][j - 1] + 1, d[i]
                              [j - 1] + 1, d[i - 1][j] + 1)
    return d[len(ref_words)][len(hyp_words)]


def grid_search_alpha_beta(model, test_audio_transform, validation_set, labels, kenlm_model_path):
    best_alpha = 0
    best_beta = 0
    ...
    for alpha in np.arange(0, 1.1, 0.1):
        for beta in np.arange(0, 1.1, 0.1):
            ...
            total_wer = 0
            for waveform, sample_rate, ref_text in validation_set:
                spectrogram = test_audio_transform(waveform)
                input = torch.unsqueeze(spectrogram, dim=0).to(device)
                output = model(input)
                decoded_text = decoder.decode(output[0].cpu().detach().numpy())
                wer = calculate_wer(ref_text, decoded_text)
                total_wer += wer
            avg_wer = total_wer / len(validation_set)
    ...
    if args.mode == 'train':
        for epoch in range(hparams['epochs']):
            ...
            torch.save(model.state_dict(),
                       'checkpoints/epoch-{}.pt'.format(epoch))
        ...
        validation_set = [
            (torchaudio.load("hcwnl.wav", normalize=True)[0], 16000,
             "he could wait no longer he could wait no longer he could wait no longer he could wait no longer he could wait no longer he could wait no longer he could wait no longer he could wait no longer he could wait no longer"),
        ]

        best_alpha, best_beta = grid_search_alpha_beta(model, test_audio_transform, validation_set, labels,
                                                       "wiki-interpolate.3gram.arpa")
        print(f'Best alpha value: {best_alpha}, Best beta value: {best_beta}')
